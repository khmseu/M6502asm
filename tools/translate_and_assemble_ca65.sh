#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
INPUT_ASM="$ROOT_DIR/BASIC-M6502/m6502.asm"
OUT_DIR="$ROOT_DIR/examples/converted"

TRANSLATED_ASM="$OUT_DIR/m6502_ca65.asm"
OUTPUT_OBJ="$OUT_DIR/m6502_ca65.o"
LISTING_FILE="$OUT_DIR/m6502_ca65.lst"
WARNINGS_FILE="$OUT_DIR/m6502_ca65.warnings.txt"
ERROR_LOG="$OUT_DIR/m6502_ca65.errors.txt"

ORIG_BIN="$OUT_DIR/m6502_original_m6502asm.bin"
ORIG_LISTING="$OUT_DIR/m6502_original_m6502asm.lst"
ORIG_VERBOSE="$OUT_DIR/m6502_original_m6502asm.verbose.txt"

mkdir -p "$OUT_DIR"

python3 "$ROOT_DIR/m6502_to_ca65.py" \
  --in "$INPUT_ASM" \
  --out "$TRANSLATED_ASM" \
  2>"$WARNINGS_FILE"

set +e
ca65 \
  -o "$OUTPUT_OBJ" \
  --listing "$LISTING_FILE" \
  "$TRANSLATED_ASM" \
  > /dev/null 2>"$ERROR_LOG"
CA65_STATUS=$?

python3 "$ROOT_DIR/m6502asm.py" \
  -o "$ORIG_BIN" \
  -l "$ORIG_LISTING" \
  -v \
  "$INPUT_ASM" \
  >"$ORIG_VERBOSE" 2>&1
ORIG_STATUS=$?
set -e

if [[ $CA65_STATUS -eq 0 ]]; then
  echo "Translation and ca65 assembly complete."
else
  echo "Translation complete, ca65 reported errors (exit $CA65_STATUS)."
fi

if [[ $ORIG_STATUS -eq 0 ]]; then
  echo "Original assembly with m6502asm.py complete."
else
  echo "Original assembly with m6502asm.py reported errors (exit $ORIG_STATUS)."
fi

echo "Translated source : $TRANSLATED_ASM"
echo "Object output     : $OUTPUT_OBJ"
echo "Listing           : $LISTING_FILE"
echo "Warnings log      : $WARNINGS_FILE"
echo "Assembler errors  : $ERROR_LOG"
echo "Original binary   : $ORIG_BIN"
echo "Original listing  : $ORIG_LISTING"
echo "Original verbose  : $ORIG_VERBOSE"

if [[ $CA65_STATUS -ne 0 || $ORIG_STATUS -ne 0 ]]; then
  exit 1
fi

exit 0
